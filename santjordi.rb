class Sketch < Processing::App
    class Writing
        def initialize(c,txt, rwidth= 50, rheight= 50)
            @c,@x,@y = c, rwidth, rheight
            text(txt,rwidth, rheight)
        end
    end
...
eot
    ...
    def draw
        ...
        @val= PFont.list()
        typo= @val[rand(@val.size)]
        size= rand(500)
        rheight= rand(@height)
        rwidth= rand(@width)-@width
        
        myFont = createFont(typo, size);
        textFont(myFont);
        #only b/w
        red_probability= random(10)
        if red_probability < 2
            c= color(204,0,0)
        else
            rgb= random(255)
            c= color(rgb,rgb,rgb)
        ...
        fill(c)
        rbegin= random(@txt.length)
        #20 chars minimum
        rend= random(@txt.length+20)

        #Writing.new(c,@txt[rbegin,rend],rwidth,rheight)
        text(@txt[rbegin,rend],rwidth, rheight)
        #
        #if @frame_count == 100
            #save('wallpaper.png')
            #exit
...
Sketch.new(:full_screen => true, :title => "santjordi")
